...
Stuck on how to answer someone? Select their message and press **⌘⇧E**. Recast suggests three different ways to reply — click one and it's copied, ready to paste. Your text is never touched.
...
**Rewrite what you're typing.** Type anywhere — Mail, Slack, Notes, a browser —
and press **⌘⇧R**. Recast rewrites your text instantly and shows a popup with
the alternatives. Click a variant to swap it in, or press **Esc** to revert to
your original.

**Get help replying.** Select a message someone sent you and press **⌘⇧E**.
Recast shows three different replies — warm, direct, asking a question,
whatever fits — each with a short label. Click the one you like and it's copied
to your clipboard; paste it wherever you're writing. Nothing on screen is
changed, and **Esc** closes the popup. This one needs an actual selection, so
...
- **Reply suggestions**: a single Messages API request returns all three
  replies, so they're written against each other and actually differ in
  approach. Picking one only writes to the clipboard — the reply flow never
  calls the replace path at all.
- **History**: stored locally at
  `~/Library/Application Support/Recast/history.json` (last 500 rewrites and
  reply suggestions). Browse/search via menu → History, clear anytime.
...
- The Claude OAuth flow is the same one Claude Code uses. It works today but
  ...
  `Sources/Recast/Rewrite/RewriteService.swift` and
